...
- general section **Import of packages, data and functions**, loading file saved by *code01_data_functions*, uploading packages used and providing some additional data wrangling (modification data to the form suitable for downstream analysis). 
  ...
  The next sections will define the specific outcome to be modelled (e.g., distance walked in an open field over 10 minutes) and will include:
  ...
  - **visualisation** with code showing
      - data
      - posterior distribution of effect size (between-groups difference)
...
For analyses which have not been performed in the previous manuscript, or were performed with too different methodologies, we used  *mu = 0* and *sigma of 2 x SD of control or SCA1 mice (the higher from these).
Priors for intercept (control SCA1 mice were set as a reference group in all models) was based to be between previous SCA1 mean from above-cited data and overall mean from current data. Sigma was set to 5*SD the outcome across groups. 
For key findings, models were fitted using various priors to estimate sensitivity of the model to prior specification. 
